...
//one way to do it
// bool mirrorShape(Node* node1, Node* node2) {
//     mirror(node1);
//     bool isMirror = similarShape(node1, node2);
//     mirror(node1);
//     return isMirror;
// }
...
bool symetricTreeFaiz(Node* node) {
    int i = 0;
    int j = node->children.size()-1;
    while(i < j) {
        //ith child hamra mirror shape hona chaye to n-1-i th child ka to make 
        //the tree symetric shape
        bool mirror = mirrorShape(node->children[i], node->children[j]);
        if(!mirror) {
            return false;
        }
        i++;
    ...
    //aur mera beech vala symetric ho
    if(node->children.size()%2 == 1) {
        bool symetric = symetricTreeFaiz(node->children[i]);
        if(!symetric) {
            return false;
        }
    }
...
//symetric tree mirror is same as symetric tree;
bool symetricTree(Node* node) {
    return mirrorShape(node, node);
}

void multiSolver(Node* node, int depth, int &height, int &max, int &min, int &size) {
    if(max < node->data) max = node->data;
    if(min > node->data) min = node->data;
    if(height < depth) height = depth;
    size++;
    for(auto child : node->children) {
        multiSolver(child, depth + 1, height, max, min, size);
    }
}

void predecessorSuccessor(Node* node, int data, Node*& pred, Node*& suc, int& state) {
    if(state == 0) {
        if(node->data == data) {
            state = 1;
        } else {
    ...
    for(auto child : node->children) {
        predecessorSuccessor(child, data, pred, suc, state);
    }
}

void ceilFloor(Node* node, int data, int& ceil, int& floor) {
    if(node->data < data) { //remove equal becox of ques kth largest
        if(node->data > floor) {
            floor = node->data;
        }
    }
    ...
    for(auto child : node->children) {
        ceilFloor(child, data, ceil, floor);
    }
}

//nlogk
void kthLargest(Node* node, int &k, priority_queue<int, vector<int>, greater<int>>& pq) {
    if(pq.size() < k) {
        pq.push(node->data);
    }
    else {
    ...
    for(auto child : node->children) {
        kthLargest(child, k, pq);
    }
}
//nk
void kthlarge(Node* node, int k) {
    int floorOf = INT_MAX;
    for(int i = 0; i < k; i++) {
        int floor = INT_MIN;
        int ceil = INT_MAX;
        ceilFloor(node, floorOf, ceil, floor);
        floorOf = floor;
    }
    cout << floorOf << endl;
...
int maxSubTreeSum(Node* node, int& max, Node*& maxNode) {
    int sum = node->data;
    for(auto child: node->children) {
        sum += maxSubTreeSum(child, max, maxNode);
    }
    if(max < sum) {
        max = sum;
        maxNode = node;
...
void prePostIterative(Node* node) {
    vector<int> pre;
    vector<int> post;
    stack<Node*> st;
    st.push(node);
...
class PairNodeChild {
    ...
    Node* node;
    int childIndex;
    PairNodeChild(Node* n, int i) {
        node = n;
        childIndex = i;
    }
};

void prePostIterativeSir(Node* node) {
    vector<int> pre;
    vector<int> post;
    stack<PairNodeChild> st;
    st.push(PairNodeChild(node, -1)); //pair bnaya hai jisme node aur usko konsa child iterate hona hai
    while(!st.empty()) {
        PairNodeChild &p = st.top();
        if(p.childIndex == -1) { // -1 ka matlan abhi yeh preorder state mai hai to pre mai daldo aur next childind pr jump kro
            pre.push_back(p.node->data);
            p.childIndex++;
        }
        if(p.node->children.size() == p.childIndex) { //is ka matlan sare child travel ho gye to pre order state hai mtlab 
            post.push_back(p.node->data);
            st.pop();
        } else {
            st.push(PairNodeChild(p.node->children[p.childIndex], -1)); //child ko travel kro matlab push to stack aur uski state -1 dalo kuki abhi voh preorder state nhi phucha hai
            p.childIndex++;
...
int diameterReturnHeight(Node* node, int& dia) {
    int h = -1; // highest height
    int sh = -1; //second highest height
    for(auto child : node->children) {
        int ch = diameterReturnHeight(child, dia);
        if(h < ch) {
            sh = h;
            h = ch;
        } else if(sh < ch) {
