...
//we also Solve this while we are returning the max and min from bottom
class PairBst {
    public:
    bool isBST;
    int min;
...
int main() {
    ...
    int min = INT_MIN;
    int max = INT_MAX;
    cout << (isBinarySearchTree(root, min, max) ? "true" : "false") << endl;
    PairBst p = isBinarySearchTree(root);
    cout << (p.isBST ? "true" : "false") << endl;
    return 0;
}