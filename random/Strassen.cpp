...
int** strassenMultiply(int** A, int** B, int n) {
    ...
    //splitting the matrixes into 4 submatrixes for both matrix A and B
    for(int i=0; i<k; i++)
        for(int j=0; j<k; j++) {
            A11[i][j] = A[i][j];
            A12[i][j] = A[i][k+j];
    ...
    //apply strassan operation on matrixes and solve subproblems
    ...
    //joining the 4  submatrixes to get the multiplied matrix.
    for(int i=0; i<k; i++)
        for(int j=0; j<k; j++) {
            C[i][j] = C11[i][j];
            C[i][j+k] = C12[i][j];
    ...
    //delete int arrays pointed by pointers
    for(int i=0; i<k; i++) {
        delete[] A11[i];
        delete[] A12[i];
        delete[] A21[i];
